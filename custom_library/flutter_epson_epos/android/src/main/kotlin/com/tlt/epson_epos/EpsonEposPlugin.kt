...
class EpsonEposPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private var printerList: MutableMap<String, Printer> = mutableMapOf<String, Printer>()
    ...
    private fun onConnect(@NonNull call: MethodCall, @NonNull result: Result) {
        Log.d(logTag, "onConnect $call $result")
        val address: String = call.argument<String>("address") as String
        val type: String = call.argument<String>("type") as String
        val series: String = call.argument<String>("series") as String
        ...
        try {
            ...
            var setPrinter = Printer(printCons, 0, context)
            Log.d(logTag, "Connect Printer w $series constant: $printCons via $target")
            try {
                val status: PrinterStatusInfo? = setPrinter.status;
                if (status?.online != Printer.TRUE && printerList[address] == null) {
                    setPrinter.connect(target, Printer.PARAM_DEFAULT)
                }
                printerList[address] = setPrinter
                resp.success = true
                resp.message = "Success Connect Printer w $series constant: $printCons via $target"
                result.success(resp.toJSON())
            } catch (e: Epos2Exception) {
                disconnectPrinter(setPrinter)
    ...
    private fun onPrint(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        val settingList = HashMap<Int, Int>()
        settingList[Printer.SETTING_PRINTSPEED] = Printer.PARAM_DEFAULT
        settingList[Printer.SETTING_PRINTDENSITY] = Printer.PARAM_DEFAULT
        ...
        try {
            printer = printerList[address]
            // retry once if printer not connected
            if (printer == null) {
                var printCons = getPrinterConstant(series)
                var setPrinter = Printer(printCons, 0, context)
                Log.d(logTag, "Connect Printer w $series constant: $printCons via $target")
            ...
            if (printer == null) {
                ...
                commands.forEach {
                    onGenerateCommandBatch(it, printer)
                }
                try {
                    val statusInfo: PrinterStatusInfo? = printer.status;
                    if (statusInfo?.online == Printer.FALSE) {
                        printer.connect(target, Printer.PARAM_DEFAULT)
                    }
                    printer.setPrinterSetting(
                        Printer.PARAM_DEFAULT,
                        settingList,
                        mPrinterSettingListener
                    )
                    ...
                    resp.success = true
                    resp.message = "Success Printing Data"
                    result.success(resp.toJSON())
    ...
    private fun disconnectPrinter(printer: Printer? = null) {
        var printerModule = printer
        if (printerModule == null) {
            printerModule = mPrinter
        }
        if (printerModule == null) {
        ...
        while (true) {
            try {
                ...
                printerModule.clearCommandBuffer()
                throw e
            }
        }
        mPrinter!!.clearCommandBuffer()
    ...
    private fun onGenerateCommandBatch(command: Map<String, Any>, printer: Printer) {
        Log.d(logTag, "onGenerateCommand: $command")
        val textData = StringBuilder()
        ...
        if (!commandId.isNullOrEmpty()) {
            ...
            when (commandId) {
                ...
                "addFeedLine" -> {
                    printer.addFeedLine(commandValue as Int)
                }
                "addCut" -> {
                    when (commandValue.toString()) {
                        "CUT_FEED" -> {
                ...
                "addTextAlign" -> {
                    when (commandValue.toString()) {
                        ...
                        "RIGHT" -> {
                            ...
                            printer.addTextAlign(Printer.PARAM_DEFAULT)
                        }
                    }
                }
            }
